 ...
 * @brief Wireless remote controller data parser
 */
...
// Remote controller state (joysticks and buttons)
struct RemoteController {
    float lx = 0;  // Left stick X (strafe)
    float ly = 0;  // Left stick Y (forward/backward)
    float rx = 0;  // Right stick X (rotation)
    float ry = 0;  // Right stick Y
    int button[16] = {0};

    // Parse raw data from wireless_remote field
